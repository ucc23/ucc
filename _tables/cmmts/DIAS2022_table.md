---
layout: page
title:  
permalink: /tables/cmmts/DIAS2022_table/
style: style
---

&nbsp;
# [Dias et al. (2022)](https://scixplorer.org/abs/2022MNRAS.512.4464D)



| Name | Comment |
| --- | :-: |
| <a href="{{ site.baseurl }}/_clusters/collinder275/" target="_blank" style="color: $blue;">Collinder 275</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/collinder345/" target="_blank" style="color: red;">Collinder 345</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/dolidze33/" target="_blank" style="color: $blue;">Dolidze 33</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/dolidze34/" target="_blank" style="color: $blue;">Dolidze 34</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso00806/" target="_blank" style="color: $blue;">ESO 008 06</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso02106/" target="_blank" style="color: $blue;">ESO 021 06</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso02602/" target="_blank" style="color: red;">ESO 026 02</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso09906/" target="_blank" style="color: $blue;">ESO 099 06</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso13954/" target="_blank" style="color: $blue;">ESO 139 54</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso14147/" target="_blank" style="color: $blue;">ESO 141 47</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso27704/" target="_blank" style="color: $blue;">ESO 277 04</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso33723/" target="_blank" style="color: red;">ESO 337 23</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso38905/" target="_blank" style="color: red;">ESO 389 05</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso39701/" target="_blank" style="color: $blue;">ESO 397 01</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso44729/" target="_blank" style="color: red;">ESO 447 29</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso45609/" target="_blank" style="color: red;">ESO 456 09</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/eso52508/" target="_blank" style="color: $blue;">ESO 525 08</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/hogg20/" target="_blank" style="color: $blue;">Hogg 20</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/loden1002/" target="_blank" style="color: $blue;">Loden 1002</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/loden991/" target="_blank" style="color: $blue;">Loden 991</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/lynga11/" target="_blank" style="color: $blue;">Lynga 11</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/lynga13/" target="_blank" style="color: $blue;">Lynga 13</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6588/" target="_blank" style="color: red;">NGC 6588</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6724/" target="_blank" style="color: $blue;">NGC 6724</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6773/" target="_blank" style="color: $blue;">NGC 6773</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6839/" target="_blank" style="color: $blue;">NGC 6839</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6840/" target="_blank" style="color: $blue;">NGC 6840</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6858/" target="_blank" style="color: $blue;">NGC 6858</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ngc6938/" target="_blank" style="color: $blue;">NGC 6938</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ruprecht114/" target="_blank" style="color: $blue;">Ruprecht 114</a> | Likely not a real open cluster. |
| <a href="{{ site.baseurl }}/_clusters/ruprecht116/" target="_blank" style="color: $blue;">Ruprecht 116</a> | Likely not a real open cluster. |


<script type="module">
import { enableTableSorting } from '{{ site.baseurl }}/scripts/table-sorting.js';
document.querySelectorAll("table").forEach(table => {
  enableTableSorting(table);
});
</script>