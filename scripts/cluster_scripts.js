...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const quickFactsBack = document.querySelector('.quick-facts-back');

  if (!quickFacts || !qualityMetrics || !quickFactsBack) return;

  const toggleFlip = () => quickFacts.classList.toggle('flipped');

  qualityMetrics.addEventListener('click', toggleFlip);
  quickFactsBack.addEventListener('click', toggleFlip);
});
...
// downloadButton.onclick = () => umami.track('cl_downl_csv');
